...
@click.option("-o", "--output", required=True, help="Output markdown file path")
def main(
    github_url: str, exclude: str, include: str, search: str, max_size: int, output: str
):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Process files
        process_repository(
            repo_dir, output, exclude_patterns, include_patterns, search_terms, max_size
        )
            ...
            for root, _, files in os.walk(repo_dir):
                ...
                for file in files_pbar:
                    ...
                    if include_patterns and not any(
                        fnmatch(rel_path, pat) for pat in include_patterns
                    ):
                        continue
                    ...
                    if search_terms:
                        with open(
                            ...
                            content = cf.read()
                            if not any(
                                term.lower() in content.lower() for term in search_terms
                            ):
                                continue
